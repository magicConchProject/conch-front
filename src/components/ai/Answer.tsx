...
export default function Answer({ A }: Props) {
    ...
    function Store(data:any) {
        if(selectedGroup) {
        toast
            .promise(addPost({ title: data.title, contents: htmlValue, group_id: String(selectedGroup.group.id) }), {
                loading: "Loading",
                success: () => "포스팅 성공",
                ...
                {A === "loading" ? (
                    ...
                    >
                        ...
                        <section className="my-2">
                            <p className="text-gray-500 text-sm px-1">{A.otherChoices?.length} 개의 추가 답변이 있습니다</p>
            ...
            {modalOpen && (
                <Modal open={modalOpen} setOpen={(open) => setModalOpen(open)} title="포스트 작성" maxWidth="max-w-[52rem]">
                    <form className="flex flex-col gap-3" onSubmit={handleSubmit(Store)}>
                                ...
                                groups.map(
                                    (data: any) =>
                                        data.joined && (
                                            ...
                                            onClick={() => setSelectedGroup(data)} key={data.id}>
                                                {data.group.name}
                                            </div>
                                        )
                                )}
                        ...
                        <HtmlEditor getChange={getChange} content={
                            `
                            <h5>ask to bard</h5>
                            ...
                            <hr/>
                            ${A.otherChoices && A.otherChoices.map((data:any, index: number) => {
                                return (`
